...
PHP/JS en cherchant un bloc PHPDoc/JsDoc en en-tête pour extraire les
métadonnées nécessaires au manifest consommé par le plugin
`up-bulk-plugins-installer`.
...
def default_file_key(path: Path, override: Optional[str]) -> str:
    ...
    return "script" if path.suffix.lower() == ".js" else "php"


def default_install_path(rel_path: Path) -> str:
    parent = rel_path.parent.as_posix()
    if parent in ("", "."):
        return "functions"
    return f"functions/{parent}"
...
def build_entry(base_dir: Path, path: Path, seen_slugs: Dict[str, Path]) -> Optional[ManifestEntry]:
    ...
    for key in files.keys():
        dest = (
            ...
            or default_install_path(Path(files[key]).parent)
        )
